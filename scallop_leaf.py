...
class Scallop_Leaf_Net(nn.Module):
  ...
  def __init__(self, provenance):
    ...
    self.base_ctx.import_file("/workspace/neuro-symbolic-dataset/scallop_leaf.scl")
...
def leaf_loader(train_dataset, test_dataset, batch_size_train, batch_size_test):
  train_loader = torch.utils.data.DataLoader(
    train_dataset,
    collate_fn=LeavesDataset.collate_fn,
...
if __name__ == "__main__":
  ...
  data_root = '/workspace/neuro-symbolic-dataset/'
  model_root = data_root
  leaf_dataset = LeavesDataset(data_root, 'leaf_11', 40, args.device)
  num_train = 330
  ...
  train_loader, test_loader = leaf_loader(train_dataset, test_dataset, batch_size_train, batch_size_test)
