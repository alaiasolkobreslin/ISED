...
class CLEVRVisionOnlyDataset(torch.utils.data.Dataset):
  def __init__(self, root: str, postfix: str = "", train: bool = True, feature_dim: int = 1024, num=100, device: str = "cpu"):
    ...
    self.questions = json.load(open(os.path.join(root, f"questions/CLEVR_{self.name}_all_color_questions_obj_{max_obj}.cropped_{num}.json")))["questions"]
...
class SceneGraphModel(nn.Module):
  def __init__(self, feat_dim, device):
    ...
    # Classifiers
    self.shape_clf = MLPClassifier(input_dim=self.feat_dim + 4, output_dim=8 + 1, latent_dim=args.latent_dim, n_layers=2, dropout_rate=0.2)
    # self.shape_clf = MLPClassifier(input_dim=(self.feat_dim + 4) * 2, output_dim=2 + 1, latent_dim=args.latent_dim, n_layers=3, dropout_rate=0.2)
  ...
  def forward(self, batched_size_vecs, batch_split):
    ...
    shapes = self.prob_mat_to_clauses(shape_prob, batch_split, all_colors)
...
class Trainer():
  ...
  def predict(self, save_image_dir):
    self.network.eval()
    iterator = tqdm(self.train_loader, total=len(self.train_loader))
    image_paths = self.train_loader.dataset.image_paths
...
if __name__ == "__main__":
  ...
  model_name = f"all_color_model_{args.learning_rate}_topbotk_{args.top_k}_dim_{args.feature_dim}_bw_{args.is_bw}_obj_{args.max_obj}_train_{args.train_num}_val_{args.val_num}_model_{args.model_layer}_latent_{args.latent_dim}"
