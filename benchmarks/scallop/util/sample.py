    ...
    def sample_train_backward(self, inputs):
      ...
      for _ in range(self.n_samples):
         ...
         if self.fn(idxs) == ground_truth:
            ...
            I_m.append(output_prob)
      I_p_mean = torch.stack(I_p) if I_p else torch.tensor(1., requires_grad=True, device=DEVICE).view(-1)
      I_m_mean = torch.stack(I_m) if I_m else torch.tensor(0., requires_grad=True, device=DEVICE).view(-1)

      I = torch.cat((I_p_mean, I_m_mean))
      I_truth = torch.cat((torch.ones(size=I_p_mean.shape, requires_grad=True, device=DEVICE), torch.zeros(size=I_m_mean.shape, requires_grad=True, device=DEVICE)))
