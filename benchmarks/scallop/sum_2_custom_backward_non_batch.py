...
class MNISTSum2Net(nn.Module):
  ...
  def forward(self, x: Tuple[torch.Tensor, torch.Tensor], y: List[int]):
    ...
    a_distrs = a_distrs.repeat((args.n_samples,1))
    b_distrs = b_distrs.repeat((args.n_samples,1))
    y = y * args.n_samples


    argss = list(zip(a_distrs.clone().detach(), b_distrs.clone().detach(), y))
    out_pred = self.pool.map(self.sampling.sample_train_backward_non_batch, argss)
