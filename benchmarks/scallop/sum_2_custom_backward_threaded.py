...
class MNISTSum2Net(nn.Module):
  def __init__(self):
    ...
    self.sampling = sample.Sample(n_inputs=2, n_samples=args.n_samples, input_mapping=[10, 10], fn=sum_2_forward, n_threads=args.n_threads)
  ...
  def forward(self, x: Tuple[torch.Tensor, torch.Tensor], y: List[int]):
    ...
    out_pred = self.pool.map(self.sampling.sample_train_backward_threaded, argss)
