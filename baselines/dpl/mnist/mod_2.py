...
def mod_2(n: int, dataset: str, seed=None):
    return MNISTOperator(
        ...
        function_name="mod_2",
        operator=lambda x: x[0] % (x[1]+1), #define solution here
        size=n,
        arity=2,
        seed=seed,
    )
...
name = "mod_2{}_{}_{}".format(method, N, args.seed)

train_set = mod_2(N, "train")
test_set = mod_2(N, "test")
...
model = Model("mnist/models/mod_2.pl", [net])
if method == "exact":
