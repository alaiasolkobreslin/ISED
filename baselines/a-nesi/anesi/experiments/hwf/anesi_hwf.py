...
from experiments.hwf.fit_dirichlet_hwf import fit_dirichlet
...
class ANeSIBase(ABC, Generic[ST], nn.Module):
    ...
    def off_policy_loss(self, p_P: Dirichlet, batch_P: torch.Tensor, eqn_len) -> torch.Tensor:
        """
        Algorithm 2
        ...
        # (batch,)
        y = self.symbolic_function(w, eqn_len)
    ...
    def train_all(self, x: torch.Tensor, eqn_len:torch.Tensor, y: torch.Tensor) -> TrainResult:
        """
        Algorithm 3
        ...
        q_loss = 0.
        data = [x_i[:eqn_len[n]] for  n, x_i in enumerate(x)]
        P = self.perception(torch.cat(data, dim=0))
        P = [P[eqn_len[:i].sum():eqn_len[:i+1].sum(), :] for i, _ in enumerate(eqn_len)]
        P = torch.stack([torch.nn.functional.pad(i, (0, 0, 0, 7-len(i))) for i in P], dim=0)
        if use_off_policy_loss or self.perception_loss == 'both':
            ...
            if use_off_policy_loss:
                _q_loss = self.off_policy_loss(p_P, P.detach(), eqn_len)
                _q_loss.backward()
                q_loss += _q_loss
    ...
    def test(self, x: torch.Tensor, y: torch.Tensor, eqn_len, true_w: Optional[List[torch.Tensor]] = None
        ...
        prior_y = self.symbolic_function(prior_predictions, eqn_len)

        successes_prior = (abs(prior_y-y) < 0.01).float().mean()
