...
class LeafState(StateBase):
    ...
    def next_state(self, action: torch.Tensor, beam_selector: Optional[torch.Tensor] = None) -> LeafState:
        y = self.y
        w = self.w
        ...
        if len(y) < len(self.y_dims):
            ...
            w = w + [action]
            oh_state = oh_state + [one_hot(action, 11).float()]
