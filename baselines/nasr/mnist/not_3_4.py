...
class MNISTNot3Or4Dataset(torch.utils.data.Dataset):
  ...
  def __getitem__(self, idx):
    ...
    return (a_img, b_img, 1 if a_digit != 3 and b_digit != 4 else 0)
...
def validation(a, b):
    ...
    predictions = torch.stack([torch.tensor(not_3_4(a[i], b[i])) for i in range(len(a))])
    return predictions
...
def final_output(model,ground_truth, args, a, b):
  ...
  for i in range(len(s_a)):
    prediction = not_3_4(s_a[i], s_b[i])
...
if __name__ == "__main__":
  ...
  model_dir = os.path.abspath(os.path.join(os.path.abspath(__file__), "../../model/mnist_not_3_4"))
