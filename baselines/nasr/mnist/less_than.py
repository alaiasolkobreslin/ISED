...
class MNISTLessThanDataset(torch.utils.data.Dataset):
  def __init__(
    self,
    root: str,
    length: int,
...
def mnist_less_than_loader(data_dir, batch_size_train, batch_size_test):
    train_dataset = MNISTLessThanDataset(data_dir, length=5000, train=True, download=True, transform=mnist_img_transform)
    ...
    train_loader = torch.utils.data.DataLoader(torch.utils.data.Subset(train_dataset, train_indices), collate_fn=MNISTLessThanDataset.collate_fn, batch_size=batch_size_train, shuffle=True)
    valid_loader = torch.utils.data.DataLoader(torch.utils.data.Subset(train_dataset, val_indices), collate_fn=MNISTLessThanDataset.collate_fn, batch_size=batch_size_train, shuffle=True)

    test_loader = torch.utils.data.DataLoader(
        MNISTLessThanDataset(
        data_dir,
        length=500,
        train=False,
        download=True,
        ...
        collate_fn=MNISTLessThanDataset.collate_fn,
...
class MNISTLessThanNet(nn.Module):
  def __init__(self):
    super(MNISTLessThanNet, self).__init__()
...
class RLLessThanNet(nn.Module):
  def __init__(self, **kwargs):
    super().__init__()
    self.saved_log_probs = []
    self.rewards = []
    self.perception = MNISTLessThanNet()
...
def validation(a, b):
    ...
    predictions = torch.stack([torch.tensor(less_than(a[i], b[i])) for i in range(len(a))])
    return predictions
...
def final_output(model,ground_truth, args, a, b):
  ...
  for i in range(len(s_a)):
    prediction = less_than(s_a[i], s_b[i])
...
if __name__ == "__main__":
  parser = ArgumentParser('less_than')
  ...
  model_dir = os.path.abspath(os.path.join(os.path.abspath(__file__), "../../model/mnist_less_than"))
  ...
  (train_loader, valid_loader, test_loader) = mnist_less_than_loader(data_dir, args.batch_size, args.batch_size)
  ...
  results_dict['task name'] = 'less_than'
  results_dict['random seed'] = args.seed
  
  dir_path = os.path.dirname(os.path.realpath(__file__))
  results_file =  dir_path + '/experiments10/less_than.csv'
