...
def mnist_mod_2_loader(data_dir, batch_size_train, batch_size_test):
    train_dataset = MNISTMod2Dataset(data_dir, length=5000, train=True, download=True, transform=mnist_img_transform)
    ...
    train_loader = torch.utils.data.DataLoader(torch.utils.data.Subset(train_dataset, train_indices), collate_fn=MNISTMod2Dataset.collate_fn, batch_size=batch_size_train, shuffle=True)
    valid_loader = torch.utils.data.DataLoader(torch.utils.data.Subset(train_dataset, val_indices), collate_fn=MNISTMod2Dataset.collate_fn, batch_size=batch_size_train, shuffle=True)
...
class RLMod2Net(nn.Module):
  def __init__(self, **kwargs):
    super().__init__()
    self.saved_log_probs = []
    self.rewards = []
    self.perception = MNISTMod2Net()
...
def validation(a, b):
    ...
    predictions = torch.stack([torch.tensor(mod_2(a[i], b[i])) for i in range(len(a))])
    return predictions
...
if __name__ == "__main__":
  ...
  model_dir = os.path.abspath(os.path.join(os.path.abspath(__file__), "../../model/mnist_mod_2"))
  ...
  (train_loader, valid_loader, test_loader) = mnist_mod_2_loader(data_dir, args.batch_size, args.batch_size)
  ...
  results_dict['task name'] = 'mod_2'
  results_dict['random seed'] = args.seed
  
  dir_path = os.path.dirname(os.path.realpath(__file__))
  results_file =  dir_path + '/experiments10/mod_2.csv'
