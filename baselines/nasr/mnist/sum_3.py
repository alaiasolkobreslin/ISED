...
def mnist_sum_3_loader(data_dir, batch_size_train, batch_size_test):
    train_dataset = MNISTSum3Dataset(data_dir, length=5000, train=True, download=True, transform=mnist_img_transform)
    ...
    train_loader = torch.utils.data.DataLoader(torch.utils.data.Subset(train_dataset, train_indices), collate_fn=MNISTSum3Dataset.collate_fn, batch_size=batch_size_train, shuffle=True)
    valid_loader = torch.utils.data.DataLoader(torch.utils.data.Subset(train_dataset, val_indices), collate_fn=MNISTSum3Dataset.collate_fn, batch_size=batch_size_train, shuffle=True)
...
class RLSum3Net(nn.Module):
  def __init__(self, **kwargs):
    super().__init__()
    self.saved_log_probs = []
    self.rewards = []
    self.perception = MNISTSum3Net()
...
def validation(a, b, c):
    ...
    predictions = torch.stack([torch.tensor(sum_3(a[i], b[i], c[i])) for i in range(len(a))])
    return predictions
...
def final_output(model,ground_truth, args, a, b, c):
  ...
  for i in range(len(s_a)):
    prediction = sum_3(s_a[i], s_b[i], s_c[i])
...
if __name__ == "__main__":
  ...
  (train_loader, valid_loader, test_loader) = mnist_sum_3_loader(data_dir, args.batch_size, args.batch_size)
